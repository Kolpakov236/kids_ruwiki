...
def _synthesis_user_prompt(
    ...
    if article_context:
        ctx_block = (
            ...
            "⚠️ СТРОГОЕ ПРАВИЛО: используй ТОЛЬКО факты из контекста выше.\n"
            "• Конкретные числа, даты, имена, названия — бери только из контекста.\n"
            "• Если факта нет в контексте — не упоминай его вообще.\n"
            "• Если контекст противоречит черновику — доверяй контексту.\n"
            "• «Интересный факт» — только если он есть в контексте.\n"
            "• Не добавляй общие знания из тренировочных данных.\n\n"
        )
    else:
        ctx_block = (
            "КОНТЕКСТ ИЗ ЭНЦИКЛОПЕДИИ: пуст.\n"
            "⚠️ Статья не найдена — объясняй только общеизвестные научные факты.\n"
            "Не упоминай конкретные имена, заведения, адреса или события которые не можешь проверить.\n\n"
        )
