...
def _llm_only_user_prompt(query: str, age: int, mode: str) -> str:
    ...
    if is_ranking:
        ...
        context_note = (
            "Ответь ТОЧНО на вопрос пользователя, опираясь только на общеизвестные, проверенные факты. "
            ...
            "СТРОГО ЗАПРЕЩЕНО: упоминать конкретных людей, заведения, адреса, события, если не уверен на 100%. "
            "Лучше дать краткий общий ответ, чем назвать непроверенный факт.\n\n"
        )
    ...
    return (
        ...
        "Структура: что это → механизм → где встречается → аналогия. "
        "Интересный факт — только если он общеизвестен и проверен. 2-3 эмодзи.\n"
